<template>
  <section id="element" class="section" style="position: relative">
    ...
    <div class="columns is-mobile is-3">
      <div
        v-for="(elem, index) in response"
        :key="elem.id"
        class="card column is-clickable"
        ...
        <div class="card-content">
          <div class="media">
            <div class="media-content">
              <p class="title is-4">{{ elem.title }}</p>
              <b-rate
                v-model="elem.vote_average"
                icon-pack="mdi"
                icon="star"
                :max="10"
                :show-score="true"
          ...
          <div class="content">
            <span class="mb-6">{{
              elem.overview.substring(0, 80) + ".."
            }}</span>
            <br />
            <br />
            <strong>Release date : </strong><i>{{ elem.release_date }}</i>
          </div>
          <b-button class="is-align-items-flex-end" type="is-success" expanded
            >Choose</b-button
          >
          <div v-if="seeresponse[index].show" class="contain-reveal">
            <div v-if="elem.id === moviecache.id" class="success-checkmark">
...
export default {
  ...
  data() {
    return {
      ...
      base_url: "",
      width: "",
      seeresponse: [{ show: false }, { show: false }, { show: false }],
      progress: false,
      valueprogress: 0,
      isLoading: true,
    };
  ...
  methods: {
    ...
    get_random_movie() {
      let rand = this.randomize_num(1, 500);
      axios
        .get(
        ...
        .then((resp) => {
          ...
          this.response.push(resp.data.results[rand]);
        });
    },
    recursive_call_ajax() {
      let rand = this.randomize_num(1, 500);
    ...
    getconfig() {
      axios
        ...
        .then((resp) => {
          this.base_url = resp.data.images.secure_base_url;
          this.width = resp.data.images.backdrop_sizes[2];
        });
    },
    get_path_img(elem) {
      return this.base_url + this.width + elem.poster_path;
    },
    revealresponse(elem, index) {
      this.seeresponse[index].show = true;
      if (elem.id === this.moviecache.id) {
        this.progress = true;
  ...
  computed: {
    response_length: function () {
      // console.log(this.response_length);
      //  console.log(this.response.length);
      return this.response.length;
    },
  },
  watch: {
    response_length(val, oldVal) {
      if (val === 3) this.response = this.shuffle(this.response);
    },
  },
};
</script>
